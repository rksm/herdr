...
Startup readiness, restoring an idle conversation, and switching agent conversations do not count as completed work. An agent that starts working immediately can finish its first task without Herdr first seeing an idle prompt. Agent responses may include `completion_seq`, identifying completed work independently of who has viewed it. A new completion matches the idle transition's `state_change_seq`. Zero identifies saved unread work restored from a previous server run and stays unchanged during agent startup. Startup does not create a completion, and session changes clear it. Older servers may omit this field. Clients connected to older servers use observed working states to guard Done badges, so they may miss a turn that starts and finishes between updates.
...
When Codex's transcript view is open (`Ctrl+T`), `prefix+e` collects the full transcript before opening `$EDITOR`. Herdr requires an idle Codex pane with the recognized transcript header and navigation controls. It collects older and newer text even when you start in the middle, then restores the original scroll position. Other panes keep their existing scrollback export behavior.

Leave the pane idle during collection. Typing, resizing, or leaving the transcript view cancels the export. If collection or restoration fails, Herdr reports an incomplete export and does not open a partial file. Large transcripts can exceed the capture timeout.
