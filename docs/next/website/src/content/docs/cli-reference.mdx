...
`tab mark` marks a tab for later without focusing it. Add `--clear` to remove the mark. Marks persist across restarts and do not change the agent or unread state for the tab.
...
`report-agent-session` updates native session identity without reporting lifecycle state. Official Claude Code and Codex hooks use `--permission-mode bypassPermissions` on session-start reports so Herdr can preserve full-permission startup mode during native restore. `release-agent` ends that source's lifecycle authority when its agent process exits.
