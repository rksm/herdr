...
| 标签页 | `tab.create`、`tab.list`、`tab.get`、`tab.focus`、`tab.rename`、`tab.move`、`tab.mark.set`、`tab.close` |
...
`tab.mark.set` 为标签页保存持久标记，并返回更新后的 `TabInfo`。该请求是幂等的，不会聚焦标签页，也不会改变其智能体状态或未读状态。
...
应用和插件应通过窗格的终端输出发送标准 Kitty 图形协议。Herdr 在兼容的外层终端中管理图像的位置、裁剪和可见性。
...
标签页事件订阅包括 `tab.created`、`tab.closed`、`tab.focused`、`tab.renamed`、`tab.moved` 和 `tab.mark_updated`。`tab.mark_updated` 包含更新后的 `TabInfo`，并且只在标记实际发生变化时发出。`tab.moved` 包含被移动的 `tab_id`、`workspace_id`、请求的 `insert_index` 和该工作区更新后的有序 `tabs` 列表。
