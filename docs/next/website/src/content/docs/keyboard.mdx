...
| Mark or unmark tab for later | `prefix+m` |
| Close tab | `prefix+shift+x` |

A star identifies each marked tab. The related Agent and Space entries show the same star. Herdr keeps the mark until you clear it. The mark is separate from agent status and the light-blue unread state. Open the Goto picker with `prefix+g`, then press `m` to show marked tabs.
