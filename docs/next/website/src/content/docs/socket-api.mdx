...
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.mark.set`, `tab.close` |
...
`tab.mark.set` stores a persistent mark on a tab and returns its updated `TabInfo`. The request is idempotent and never focuses the tab or changes its agent or unread state.

```json
{"id":"req_mark","method":"tab.mark.set","params":{"tab_id":"w1:t2","marked":true}}
```
...
Official Claude Code, Codex, and Hermes Agent integrations include `permission_mode` on session-start reports. Herdr uses `bypassPermissions` to preserve full-permission startup mode during native restore.
...
`tab.renamed`, `tab.moved`, and `tab.mark_updated`. Herdr emits
`tab.mark_updated` only when the mark changes. The event carries the updated
`TabInfo`. `tab.moved` includes the moved `tab_id`,
