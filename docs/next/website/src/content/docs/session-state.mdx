...
Saved unread marks survive server restart. Agent startup keeps those marks and does not mark previously read agents as unread. Reading a restored pane clears its mark. The next completed task can mark it unread again.
...
Claude Code integration version `8`, Codex integration version `7`, and Hermes Agent integration version `4` also report whether a session started in full-permission mode. Herdr restores those sessions with `claude --resume <id> --dangerously-skip-permissions`, `codex resume --yolo <id>`, and `hermes --resume <id> --yolo` respectively. Sessions started in another permission mode keep the standard resume commands.
