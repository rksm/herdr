...
| タブ | `tab.create`、`tab.list`、`tab.get`、`tab.focus`、`tab.rename`、`tab.move`、`tab.mark.set`、`tab.close` |
...
`tab.mark.set` はタブに永続的なマークを保存し、更新後の `TabInfo` を返します。リクエストは冪等であり、タブのフォーカス、Agent 状態、未読状態を変更しません。
...
アプリケーションとプラグインは、ペインのターミナル出力を通じて標準の Kitty グラフィックスプロトコルを送信してください。Herdr が対応する外側ターミナルで画像の配置、クリッピング、表示状態を管理します。
...
タブのイベント購読には `tab.created`、`tab.closed`、`tab.focused`、`tab.renamed`、`tab.moved`、`tab.mark_updated` があります。`tab.mark_updated` は更新後の `TabInfo` を含み、マークが実際に変わった場合だけ発行されます。`tab.moved` は、移動した `tab_id`、`workspace_id`、要求された `insert_index`、そのワークスペースの更新済みの順序付き `tabs` リストを含みます。
